...
- **CineKind opens on a wall of the honourees that goes by as you
  scroll.** The page used to open on the film, muted, with the honourees
  passing underneath on a film strip. It now opens the way a picture
  house does: on the hoarding. The eleven 2026 honourees, and nothing
  else, pasted up as photographs, each in a tall crop and a close one so
  the wall reads as posters of the same people rather than one
  photograph repeated. The wall is a shallow dome: every poster is
  turned and set back in proportion to its distance from the middle of
  the screen, so a face is flat and near at the centre and turns away
  toward the edges, and turns as it travels from one to the other. It
  has no edge in any direction, being laid out as strips that wrap, and
  the section is a track a little over two screens long with the wall
  pinned under the header, so the page's own scroll is the camera: as
  the page goes by the wall climbs and drifts a little across, eased so
  it moves like something with weight, until the track ends and the
  roll arrives underneath. A mouse can take hold of the wall and throw
  it, a trackpad's sideways swipe pans it, and nothing hijacks the
  wheel: the page still arrives where it always did. The wall is bright
  at rest; the wash comes down when a pointer is on it and lifts off
  the face under the pointer, whose name and award take the corner
  where the title stood. Shuffle pastes the wall up again, poster by
  poster, as it turns to a fresh tilt.
- The twenty-two posters are markup, in a composed order and laid out
  by the stylesheet as flex rows, so the wall stands without a script,
  is read by search, and is walked by a keyboard one poster per Tab; the
  strips put the originals first, in tab order, and everything after is
  an aria-hidden copy, with no one pasted next to themselves. A focused
  poster that is not on the stage is brought to the middle of it by
  moving the wall, not by scrolling the stage off its pins. The corner
  caption mirrors the poster's own, which keeps its text for the
  accessible name. A photograph that does not arrive comes off the deck
  and the wall is laid out again. On a screen with no hover the first
  touch lights a poster and the second follows it. A reduced-motion
  request gets the wall standing still on a section one screen tall: no
  track, no camera, no throw.
- Every poster leads to that person's line of the 2026 roll, which
  answers by lighting. The film is a button beside Shuffle. The
  screening it opens no longer autoplays muted behind the page: the
  embed mounts with sound on the press, as a gesture allows, and is torn
  out on Exit, which is how the film stops.
- The film strip (reel26) and the muted hero are gone with their styles.
  The 2026 roll now sits directly under the wall, the trophy after it;
  the about, kind set and 2025 sections are untouched. Search index
  rebuilt.

## v1.304

- **The plot is gone again, and this time the lesson is written down.**
  v1.303 put the eighty units on the page as a plot with each one joined
  to its nearest neighbour, betting that the web would make the scatter
  read as a country. It did not; the owner's review was two words. That
  makes two attempts and two removals with the same verdict, so the
  guard test now names both generations of the machinery and says no
  third attempt should reach a review: the numbers, the list and the
  search are the page. The setup path from v1.303 stays; that part
  serves.

## v1.303

- **The units draw themselves.** The directory's eighty coordinates were
  sitting unused in the data, so the page now opens them as a plot: a
  full ink band where every unit is a glowing point at its true position,
  each one an anchor into its own row, with the city's name appearing on
  hover or focus. No tiles, no third party, and no border of any kind
  drawn by this site. An earlier scatter was removed for reading as dots
  rather than as a country; this one joins every unit to its nearest
  neighbour first, so the network reads as what it is, one organisation
  joined up across the map, and the guard test keeps the old lesson: no
  scatter in the hero, ever again.
- **Starting a unit is on the page in full.** The path from PFA's own
  Set Up A PFA Unit page, transcribed in its order: membership, the
  six-member board, the form and covering letter, the chairperson's
  approval, the jointly operated bank account with the trust deed on
  request, and the monthly members list and work report, down to the
  best-unit prize. Beside it, the first month: one campaign, a vet in
  the group, rescue before posters, and the laws in every member's
  pocket. The application form links to the PDF the head office serves,
  and the address stays as that page gives it.

## v1.302

- **The words arrive with the colour.** The home page now opens as the
  photograph alone, black and white, and the first movement brings both
  at once: the picture warms while the headline rises into place, its
  letter-spacing settling as it lands, with the rule, the line and the
  two doors following half a beat later. The veil over the text is put
  on by the script, so a page with no JavaScript shows everything from
  the start; the veil carries no transition, so applying it is invisible
  even if a slow load paints the text for a frame; and a key press
  counts as arrival, so a keyboard visitor is never tabbing into
  invisible doors. Reduced motion gets the appearance without the
  choreography.

## v1.301

- **The home photograph wakes at the first movement.** The page loads
  black and white, and the first movement of the mouse blooms it into
  colour over two seconds; a touch or a scroll does the same on screens
  without a cursor. One-way on purpose: a room, once lit, stays lit. A
  reduced-motion request gets the change of state without the animation,
  and with scripts unavailable the page simply stays monochrome, which
  is where it started.

## v1.300

- **Send food to a place is gone, whole.** The chooser, the four-step food
  flow, the item catalogue, the destination picker, the live order summary
  and the cart bar left donate.html; parseSend, the SEND_CATALOG, the items
  parser, the PFA-SND order prefix, the send receipt rows and the send arm
  of every type list left the server; and the Gift option left the admin
  purpose menu. The donation page is one flow now, shown without being
  chosen. Historical send transactions, if any exist, stay readable in the
  admin register under Every purpose; only the rails are gone.
- **The tests moved with the code, not away from it.** The send pricing,
  receipt and form tests retired with the feature. The district suite was
  the one worth saving: the Punjab-with-Udupi fault it guarded did not
  retire, so donate-district.test.js became district-picker.test.js and now
  holds the caregiver application's picker to the same law, ending with the
  server refusing the pair the picker prevents. The PIN inference and
  campaign-link tests retired with the picker that had those features, and
  the file says so.

- **A paid food order still settles.** Removing send from the callback's
  accepted types would have refused the settlement of an order paid before
  the removal, telling CCAvenue the stored transaction could not be found.
  The callback accepts a stored send transaction again and issues the
  generic receipt: no new rails, but the ledger is honoured. The removal
  had also cut a media-query opener out of donate.html's stylesheet, which
  orphaned the mobile rule the live form still uses; the sheet parses
  clean again.
- **The home page holds its breath.** The hero photograph loads black and
  white, exactly as before, and the first movement of the mouse blooms it
  into colour over two seconds. A touch or a scroll does the same on
  screens without a mouse, and the listeners remove themselves after the
  single use.

## v1.299

- **The home page is the hero.** The essay doorway came out, so the page
  is the photograph, the headline and the footer. The essay itself stays
  whole at someone.html, reachable from the quiz page's two links and
  from search; only the doorway on the front is gone.
- **The hero's second door opens the units directory.** Report cruelty
  gave its place to Find a unit, which is the thing a person standing
  next to an animal most often needs first. Reporting stays one tap away
  inside the site.

## v1.298

- **During a screening, stillness clears the room.** 2.6 seconds without
  the mouse and the Pause and Exit buttons fade off the picture, and the
  drawn cursor sleeps with them, so nothing sits on the film. Any mouse
  movement, key or touch wakes both at once, and leaving the screening
  restores everything. The listeners are guarded to the screening, so on
  the ordinary page the whole mechanism is inert.

## v1.297

- **The reel is complete.** YODA, Priya Hebbar and Pooja Sakpal
  (Compassionate Care, 009), take the last empty frame, and every one of
  the eleven 2026 honours now passes on the strip with a portrait, in
  roll order, 001 to 011. Eleven frames per set, retimed to 116 seconds
  so the drift never changed pace from the day it was four.

## v1.296

- **Ten of the eleven honours carry a portrait on the reel.** Kalyan Varma
  (Kindness in Frame) and Rajan Shahi (Values in Action) join the strip in
  roll order, spliced into all four sets. Ten frames per set is a quarter
  more travel per loop than eight, so the duration moved from 84 to 105
  seconds and the drift keeps its pace. Only YODA (009) waits for a
  picture now; the full roll below the strip has carried all eleven names
  since v1.291.

## v1.295

- **Four more frames on the reel.** Amit Rai, John Abraham, Raveena Tandon
  and Riteish and Genelia Deshmukh join the strip with their portraits,
  spliced in roll order between the four already passing, so eight of the
  eleven honours now carry a picture. Converted to 720px webp at 37 to
  82KB. Twice the frames per set means twice the pixels per loop, so the
  duration doubled to 84 seconds and the drift keeps the same pace on the
  screen.

## v1.294

- **The honourees pass as a reel.** The 2026 portraits ride a continuous
  strip of film: ink stock full-bleed across the viewport, perforations
  translated with the pictures so the sprockets and the frames are one
  strip, four identical sets looping a quarter at a time so the seam can
  never be on screen. Hovering or tabbing into the strip holds the frame;
  a frame under the pointer comes up to full saturation. No script: the
  projector is CSS, so it cannot fail to load. A reduced-motion request
  swaps the projector for a hand-wound strip the person scrolls, with the
  duplicate sets removed so nothing repeats and every honouree is still
  reachable.

## v1.293

- **The card carries the chairperson's signature, at last.** The renderer
  has always reserved the space and refused to draw a signature on
  anything but an issued card, but media/pfa-signature.png did not exist,
  so no card ever carried one. The signature was lifted from the supplied
  card artwork with its transparency intact, trimmed, and installed. Every
  card issued from the register now signs itself: Smt. Maneka Sanjay
  Gandhi, Chairperson, People For Animals, exactly as the artwork has it.
- **The face prints the agreed fields.** Name in the header, then ID
  number, district, state and issued on, with the photograph and the QR
  that makes a card checkable. The address and contact stay in the record
  for review and delivery; they no longer print on the front. District and
  state are picked from the same geography the donate flow validates, ride
  the application payment, and persist through issuance to the public card.
- **The photograph is the only document asked for.** The proof of address
  came out of the form, the documents route and the fee gate; a proof
  still arriving from an old tab is kept beside the photo rather than
  refused, so nobody mid-application loses their work. Applications land
  in the payments register as before, and cards are issued from the admin
  panel, which is where the register already reads them.

## v1.292

- **The essay left the landing page.** Everything between the hero and the
  footer of the home page was one long piece: Someone, not something, eight
  chapters on what the science says about the animals we eat, ending in the
  quiz and the Where-this-comes-from panels with their Supreme Court
  paragraph and Read the sources link. It is a Learn essay, and it now
  lives at someone.html whole: the same chapters, the same slot crops, the
  same quiz, the same sources tail, under the site chrome with Learn
  marked as its section. The home page keeps the hero and gains one
  doorway band that names the essay and opens it.
- **Everything that pointed at it moved with it.** quiz.html's two links
  to the findings follow the essay; the crop-anchor tests and the
  quiz-tile test read someone.html now, since they describe markup that
  moved; the chrome stamper, the SEO builder, the sitemap, the search
  index and the header-consistency vocabulary all know the new page. The
  slot component's styles and the story-rail arrows left the home page
  with the sections that used them.

## v1.291

- **The 2026 honours are on the page, above the 2025 record.** Eleven
  honours, announced ahead of Mumbai on 4 October: Anant Ambani, Amit Rai,
  Nitin Jain, John Abraham, Raveena Tandon, Wamiqa Gabbi, Kalyan Varma,
  Riteish and Genelia Deshmukh, YODA (Priya Hebbar and Pooja Sakpal),
  Nikhar Juneja and Rajan Shahi. Four supplied portraits are featured,
  converted to 720px webp at 33 to 48KB each, in media/cinekind-2026. The
  featured cards carry name and honour only: no citations were supplied,
  and the page invents nothing. The inaugural 2025 roll stays exactly as
  it was, because it is a record.
- **The About block stopped counting one edition.** It described ten
  honours given once a year; with eleven more it now describes the roll,
  and the facts line counts 21 across two editions.

## v1.290

- **Sound on is now a screening.** Pressing it restarts the film from the
  top with full volume, fades the title card and the scrim away, and pins
  the picture over the whole viewport: chrome cleared, page scroll held,
  Pause and Exit in the corner, Esc to leave. Leaving mutes the film and
  returns the title card. The reduced-motion Play button screens the same
  way, since its press already starts the film with sound.
- **IMAX without the Fullscreen API, for the cursor's sake.** A
  fullscreened element paints above every z-index there is, including the
  drawn cursor's layer, which is why the pointer vanished in browser
  fullscreen. The takeover is in-page at z-index 70 instead: above the
  announce bar and the header, under the theatre, and infinitely under
  the cursor layer, so the drawn pointer survives the whole screening
  untouched.

## v1.289

- **The title card no longer depends on the footage being dark.** The film
  behind it opens on a white press-conference wall, and the scrim went
  fully transparent through the middle, so the eyebrow drowned into the
  wall and the headline's first line rode on luck. The scrim is now a
  plate: a left gradient behind the text column and a bottom gradient that
  never reaches zero, with real shadows under the eyebrow, the headline
  and the meta line. Legibility now comes from the card, not from hoping
  the film cooperates frame by frame.

## v1.288

- **CineKind opens on the film, not on a paragraph.** The page's ceiling is
  now the CineKind film itself, full screen under the chrome, muted and
  looping from the moment the page loads, with the title card set over its
  lower edge and three buttons in the corner: Sound on, Pause, and, where
  it applies, Play. Muted because a browser permits autoplay no other way;
  the Sound button is the person's own gesture, which is what unmuting
  legally is. Someone who asks for reduced motion, or is on a data-saver
  connection, gets the poster and a Play button instead, and their press
  starts the film with sound. The poster sits underneath throughout, so an
  unreachable player leaves a picture rather than a hole.
- **Within the rules the site already keeps.** The embed is
  youtube-nocookie, already allowed by the CSP the site ships; the frame
  takes no pointer events, so the page scrolls over the picture and
  YouTube's surface is never the interface; and cinekind leaves the shared
  text-hero family by name, since a screen is a different composition from
  a headline, and the family rule stays meaningful for the twelve pages
  that keep it.

## v1.287

- **Every form was walked to the admin register, and one could hide.** The
  audit: Ask Maneka (PFA-Q), careers (PFA-J), cruelty reports with their
  photographs (PFA-CR), wall stories (PFA-S), event requests (PFA-EV) and
  volunteer applications (PFA-V) all post through pfa-forms.js to
  /api/pfa-submissions, land in the submissions register newest-first with
  no kind excluded, and can never show success without a server reference.
  Donations, gifts, card postage and the caregiver application fee all land
  in the payments register. Documents and photographs travel beside their
  records and open through the admin attachment route. Nothing on the site
  mails a form anywhere or pretends to send one.
- **The one gap: the application fee could not be filtered.** The payments
  purpose menu offered donations, postage and gifts, but the caregiver
  application fee is its own type and the filter matches types exactly, so
  "Colony card postage" silently excluded application money and no menu
  entry could select it. The purpose menu now carries it. Also corrected: a
  stale comment on cinekind.html claimed the page posts nominations; its
  form was removed when the 2026 awards went invite-only, and the comment
  now says so instead of describing wiring that is not there.

## v1.286

- **Calicut is reachable on the numbers it publishes today.** Unit 19's
  entry carried the head's number and an alternate, 7034659391, that the
  unit's own page no longer lists. Re-read from
  peopleforanimalsindia.org/units/details/19 on 12 Sep 2026: Mr. Madhu
  Nair's number stays first, the stale alternate is gone, and the three
  members the page now names (Shymap, Jinsha K and Jishnu, the joint
  secretary) have their numbers carried across, so a caller has four ways
  in instead of one that works and one that may not. The head's name and
  the unit email were already right and stay as they were.

## v1.285

- **The cinema theme did not belong to this site.** v1.284 dressed the
  Academy in the landing page's system: warm paper, acid, compressed
  nine-hundred-weight display, mono labels. On its own it held together;
  beside nineteen pages of Marcellus and hairlines it read as someone
  else's page wearing this site's header. Reverted whole to the v1.283
  Academy: the screening room, the merged triage and the mosaic stay,
  set back in the house face on the house palette.
- **The revert is a new version, not a rewind.** The stamp moves forward
  to v1.285 and this entry stays above v1.284, so the deployment list
  says what happened in order. A stamp that went backwards would be the
  "which build is live" hole this script was built to close. The
  chrome-only exception for academy.html leaves test/fout.test.js with
  the theme that needed it; the page leads its stacks with Marcellus
  again.

## v1.284

- **The Academy wears the cinema theme now.** From the landing page handed
  over as the reference: warm paper and near-black ink in alternation, an
  acid ticker as the one loud line, compressed nine-hundred-weight display
  set at poster scale with serif inflections dropped mid-headline, mono
  kickers numbered 01 to 05 down the page, 1px strips for the triage and
  the films, film grain over everything, and the acid on the triage's
  hover, the buttons and the selection. The teaching half keeps its paper
  and its red-and-green washes, because a dose table is still a dose table.
- **What the theme could not take.** The stamped chrome, the drawn cursor
  and the hero family's shared headroom stay as the site has them; the
  theme's Google-hosted faces became system stacks, because this site's
  promise is that nothing third-party loads; and its em dashes became
  slashes, because no page here shows one. Marcellus stays loaded for the
  chrome and the theatre, and test/fout.test.js now records academy.html
  as a chrome-only page for it.

## v1.283

- **The films are a screening room now.** The band goes to ink, the heading
  is set at poster scale in a two-line lockup, and the five stills close up
  into one solid block: 3px gutters, the short standing two rows tall, the
  captions moved onto a scrim inside each frame with the play control in
  the middle of the picture, where a cinema puts it. The credit is set as a
  label under the title, so whose film it is reads inside the frame itself.
- **The dark opening act was raised to match.** The hero and triage display
  sizes come up so the page opens at the same scale it screens at. The
  teaching half stays on paper deliberately: a dose table read in a hurry
  is owed daylight, and the alternation of ink and paper is now the page's
  rhythm rather than an accident of its history.

## v1.282

- **The academy page opened by asking the same question twice.** Six
  emergency tiles in the hero and six more in the triage below it, both on
  ink, four targets the same, with the standing note squeezed to a sliver
  between the two. One door now: the hero hands straight to the triage,
  which inherits the icons and the lesson references, and the two targets
  that lived only in the hero (bitten yourself, holding a prescription)
  stand under it as a line. The note moved below the triage, to where the
  page turns from acting to learning.
- **The film grid no longer breaks around the short.** A 9:16 film in an
  equal-height row stretched the row to fit and left the other four captions
  hanging over a void. The five films are a mosaic now: three columns, the
  short standing exactly two rows tall, every edge meeting another. The
  number chip sits on ink so it survives a white poster, and on a phone the
  short takes its 9:16 frame back.
- **The syllabus index carried two generations of its own CSS.** .mod__n and
  .mod:hover were each defined twice, and the later pair dropped the numeral
  column's span, width and tabular figures, so ten rows almost lined up. One
  generation remains. The heading also repeated itself word for word in its
  own standfirst, and the films heading said Watch twice; each now says it
  once.
- **Triage tiles route like the syllabus cards.** The deep-link router
  listened for the hero tiles and never for .trg, so clicking the target for
  a module already open did nothing. The three-across grid also replaces
  auto-fit, which found five columns at 1440px and left the sixth tile
  orphaned on a row of its own.

## v1.281

- **A real title with an apostrophe stopped the ship.** npm run media:films
  wrote "Why Don't Snakes Poison Themselves?" into academy.html the only way
  a single-quoted literal can hold it, with the apostrophe escaped, and the
  suite then demanded the tile show the backslash too. The page was right:
  filmsOf() in test/academy-films.test.js read the source without decoding
  the literal, so a correct title failed against the very tile that showed
  it correctly.
- **The test now decodes what it reads.** The escapes are undone before any
  comparison, so the invented-title rule and the tile rule both judge what a
  visitor sees rather than what sits in the source. This also closes the gap
  where an invented or misattributed title carrying an apostrophe would have
  slipped past the rule named for it. Verified in both states the rules exist
  for: 19 of 19 before the fetch writes titles, 625 of 625 after it.

## v1.280

- **The build command still named a script the shop removal deleted.**
  vercel.json ran `node scripts/build-catalog.js && node scripts/minify.js`,
  and build-catalog.js went with the shop it fed. 532 tests, the linter, the
  site audit and the chrome check were all silent, because not one of them
  read that field; the first thing to notice was Vercel, which stopped before
  the build began with `Cannot find module '/vercel/path0/scripts/build-catalog.js'`.
  The command is now `node scripts/minify.js`.
- **And a test so no command can name a file that is not there.**
  test/commands-exist.test.js reads vercel.json's build, install and dev
  commands and every npm script, pulls out each local .js or .sh handed to
  node, bash or sh, and fails with the command named if the file is missing.
  It reproduces this failure exactly when the old command is put back. A build
  that cannot start is the most expensive way to find a missing file, because
  production is the only place it is found.
- **The build is now run, not just reasoned about.** `node scripts/minify.js`
  writes dist/ with 21 pages and no trace of the shop. 535 tests pass.

## v1.279

- **The shop is removed.** Gone: pfa-shop.html, shop.html and product.html; the
  bag, the brand strip and the catalogue snapshots; 19 server modules; 16 API
  routes, including every Shopify and Razorpay webhook; the /products/:handle
  rewrite in both Vercel and Firebase and the product-page function template;
  the nightly store-reconcile cron and its GitHub workflow; the Store register
  in the admin panel and in lib/routes/admin/records.js; the Cart control and
  the paw tracks that walked under the Shop nav link; the Shop rows and the
  product snapshot in site search; five build scripts, three setup documents
  and 24 test files. 21 pages and 24 API routes remain, from 24 and 40.
- **What was kept, and why.** Donations were not touched: donate.html uses
  /api/payment/create and the transactions collection, not the store's. The
  caregiver card, submissions, search and the rest of the admin panel are
  untouched. quiz.html stays: it is an editorial quiz that happened to be
  generated from the shop's shell. track.html stays and still follows a report,
  a question or an application; only the order half was removed, along with the
  now-dead #tItems element and its styling.
- **Two things the removal exposed.** scripts/ship.sh read the release version
  out of pfa-shop.html, so deleting that page would have broken the only route
  to production; it and scripts/doctor.sh now read index.html. And
  lib/store-payments.js turned out to be shop-only after all: a first
  dependency pass called it shared because the admin store register reached it,
  and nothing else did.
- **PFA-RX, the prescription submission, went with the shop.** Its spec named
  product.html as the page that sends it, and no other page ever did.
- **Left alone on purpose.** _inline-extracts/admin.inline.js is a read-only
  snapshot of admin.html and now mentions a Store screen that has gone. The
  folder's own README says these go stale and may be ignored, and it already
  holds snapshots of pages that no longer exist. The shop's own extract was
  deleted with the rest of the shop. 532 tests pass, lint is silent, the site
  audit finds no dead links, and the chrome is in sync on every page.

## v1.278

- **The Academy joins the editorial theme, and ACADEMY stops shouting.** The
  word above the headline was set at 700 weight and .14em where LAWS, UNITS and
  the rest sit at 400 and .16em, so it read bold beside its siblings. The cause
  was not the eyebrow rule, which is identical on every page: it was that
  academy.html never linked assets/pfa-theme.css. The Sep 2026 theme pass put
  the editorial layer - label weight, display leading, the numbered band head,
  the call-to-action control, square corners, hairlines - in one file loaded
  after each page's own style, "on every chrome page". Academy was missed, so
  its own 700 stood unretuned. Linking it moves four things into line with the
  other nineteen pages: the eyebrow to 400 and .16em, the display to a leading
  of 1 and .005em tracking, the two closing bands to the numbered head with its
  hairline, and the buttons to the fixed 50px control. Measured against
  laws.html afterwards, every one now computes identically.
- **Two tests so the family cannot drift again.** test/hero-alignment.test.js
  already pinned the room reserved above the headline; it now also requires
  every page in the hero family to link the theme, and to link it after its own
  <style>, since loading it first would lose it every tie. Both name the page
  that broke.
- **Noted, not fixed: product.html and quiz.html are missing the theme too.**
  A different cause, and a separate job. Both are generated from pfa-shop.html
  by scripts/build-product-template.js and build-quiz-template.js, which copy
  the head only as far as the first </style>; the theme link sits after it, so
  it has never been carried across. They are not in the hero family, so the new
  test does not cover them. 857 tests pass.

## v1.277

- **The Academy hero starts where every other one does.** Thirteen pages open
  on the same dark editorial hero, and the room reserved above the headline is
  meant to be identical on all of them so the title does not jump as a reader
  moves between them. Rewriting the Academy's stylesheet in v1.275 brought its
  own numbers with it: 60px above the headline against everyone else's 72px, 32
  against 36 on a phone. The page started twelve pixels high, which is exactly
  the sort of difference that is invisible on one page and obvious the moment
  you flick from Laws to Academy. The four numbers now match the other twelve
  pages: 72px and 64px on a desktop, 36px and 48px on a phone.
- **And a test now says so.** `test/page-shell.test.js` only ever checked that
  the rule mentions `--ann` and `--nav`, not what it adds on top, so nothing
  caught the drift. `test/hero-alignment.test.js` reads the first section of
  every page, keeps the thirteen whose first class is `hero`, and fails with
  the page named if any of them reserves a different amount. The shop, a
  product page, the quiz and units have their own first section and are not in
  the family. 855 tests pass.

## v1.276

- **The Academy opens on one module, not on all ten.** v1.275 fixed the wrong
  half of the problem. Putting each lesson's key sentence in its closed card
  cured "nothing at once" and bought it with scroll: the page measured 41,221px
  on a phone, 48.8 screens, 85% of it lessons nobody had asked for yet. The
  filter that switches modules already existed and already worked; it just
  started at "all". It now starts at Foundations, and the chips, the ten
  syllabus cards, the triage tiles, the hash and a new previous/next control at
  the end of each module all move between them. Searching widens to the whole
  Academy and gives the module back when the box is emptied; "All lessons" is
  still one press away, and with no script every module still shows, as before.
  A phone is now 7,548px, 8.9 screens, and a desktop 6,952px, 7.7 screens.
- **The self-check is offered rather than served.** Twenty-six questions
  rendered open cost 7,800px on a phone for a test nobody had asked to sit. The
  heading stays and the questions wait behind "Answer 3 questions".
- **The source list is offered too.** That every lesson rests on a named
  document is this page's credibility and stays in the heading; the eight
  documents behind it are reference, and were 1,500px of tail.
- **The discovery card moved below the lessons.** At the top it was 350px
  between arriving and the first lesson; after a module it is a way onward.
  With the module aims set as a swipe on a phone, the distance to the first
  lesson fell from 3.3 screens to 2.8.
- **Smaller corrections.** The end-of-module control never shows a blank slot:
  the first module offers the syllabus where a previous one would be, the last
  where a next one would. "The rules moved" lays its six cards three across
  rather than five and an orphan. Closed cards, the closing bands and the
  source entries are all tightened. 853 tests pass; the page is 279KB.

## v1.275

- **The Academy shows its point before it is opened.** The teaching was right
  and the presentation was not: 76 collapsed rows all looked the same, so a
  scroll down the page taught nothing and every fact cost a click, and the row
  that was opened then poured out four to six paragraphs, lists, tables, a "Do
  this" and a "Careful" at one weight, with the two boxes that prevent harm at
  the very bottom. Nothing at once, then everything at once. Each lesson's own
  key sentence (`.qa__lead`, already written, already marked) now sits in the
  closed card with badges counting what is inside, so scrolling is worth
  something on its own; opening one gives Do and Never first, then the lesson's
  list items as a grid of fact cards, then ordered lists as numbered steps,
  then one "Go deeper" toggle holding the prose and the tables. Nineteen
  lessons are prose and nothing else and show it directly rather than behind a
  toggle, because a card whose only content is a button is a dead end. Not one
  word of the copy was rewritten: every block was moved, relabelled or
  re-wrapped, and the rebuild was checked word by word against the previous
  file, which lost nothing but the colons that became a fact card's title
  break.
- **The Academy opens on the emergency, not on Module 01.** The hero was a
  70-word paragraph over a 100-word note over ten dense cards, roughly 400
  words before the first lesson. It is now a short line and six triage tiles -
  hit by a car, snakebite, poisoning, heatstroke, the caregiver's own bite, a
  prescription in hand - each linking straight into the lesson that answers it,
  so the most urgent content is one tap from the top instead of seven modules
  down. A discovery card under the note draws a random lesson's title and key
  sentence out of the page itself, so it can never quote something the Academy
  no longer says.
- **Colour on the Academy now means something.** `--go` (#146b3a) is new: "Do
  this" was pure black and read exactly like body copy, which left the most
  actionable element on a page about animal medicine with no visual weight at
  all. Red is harm, green is do this, blue is optional depth, and nothing is
  tinted for decoration. A per-module hue was drafted and cut for competing
  with it.
- **The deeper layer is not a nested `<details>`.** Two reasons: the suite's
  lesson regex is non-greedy, so a nested `</details>` would cut every lesson
  body short and take the source chips with it; and a lesson's prose now ships
  visible and is closed by the page script, so with no script it sits where it
  did before rather than behind a button that cannot open. Nothing flashes,
  because the lesson around it is shut until long after the script has run.
- **A filter no longer throws away what was open.** `apply()` closed every
  lesson on every keystroke; it now closes only the ones it is about to hide.
- **Smaller Academy corrections.** The ten module cards carry the numeral
  alone: "Module 01" set as a 46px display numeral filled the card and squeezed
  the title into a column three words wide. "You will be able to" is printed
  once above the aims rather than on all four boxes. The lesson column is
  capped at 880px so the disclosure sign is not marooned 200px from the text.
  Six triage tiles sit three across, which is two even rows rather than five
  and an orphan, and become compact rows on a phone, where six tall cards cost
  1500px of scrolling before the page began. The module grid becomes a
  scroll-snapped swipe under 720px. 853 tests pass and the page is 270KB,
  inside the 400KB ceiling.

## v1.274

- **An Academy, ten modules long.** academy.html teaches how animal medicine
  works, for India: how to read a label, a prescription and a dose; the
  drug classes a caregiver meets and what each is known to break; antibiotics
  and the laws that now fence them (Schedule H1, the 2019 colistin ban, the
  FSSAI residue limits); vaccines, with the WSAVA 2024 series for dogs and
  cats and the NDDB, NADCP and ICAR-CIRG schedules for cattle, buffalo, goats
  and sheep; antivenom and snakebite; immunoglobulins, including the whole
  NCDC post-exposure protocol for the person who was bitten; first aid, from
  the sixty-second check to milk fever; dosage arithmetic, with a checker that
  multiplies and divides and deliberately knows no drug; contraindications
  and interactions in two tables; and the Do's and Don'ts, with an audit of
  what Indian kitchens put on wounds. Seventy-six lessons, each on a named
  source with a chip that filters the page to everything resting on it; a
  short self-check after each module; the laws page's search, filter and
  expand mechanism reused with a lesson vocabulary. The standing note under
  the hero says what the page is: information beside a vet, never in place
  of one, and every figure on it is a published label range, not an
  instruction. The four veterinary NSAID bans (diclofenac 2006, ketoprofen
  and aceclofenac 2023, nimesulide 2025) are stated with their dates, which
  test/academy.test.js pins alongside the lesson and quiz structure.
- **The deployment can no longer pick up the wrong Firebase key.** On 7 Sep
  the ship script chose the service account by `ls -t`, the newest
  `*firebase-adminsdk*.json` on the Desktop, and that was the key for
  pfa-oldsite, a project this site left. The sign-in check caught it
  ("browser signs in to pfa-new-website, server holds pfa-oldsite") and
  nothing was pushed, but only after the working tree had been replaced and
  the admin claim granted on the wrong project. Now: scripts/firebase-project.js
  reads every place the tree names a project (assets/firebase-config.js,
  .firebaserc, package.json) and the project_id of a key, and exits non-zero
  unless they all agree; ship.sh finds the key by that project, never by its
  date, refuses a key for a retired project outright, and runs the project
  check, the claim grant, the sign-in check and the tests in the unpacked
  tree before the live tree is backed up and replaced. `PFA_SERVICE_ACCOUNT`
  names a key outright; `npm run check:project` runs the comparison on its
  own. check:admin now names a retired project as its first failure and
  checks the deploy targets against the browser. The tree still names
  pfa-new-website everywhere, which is the project the named key belongs to;
  the fix was to the choosing, not the configuration.
- **Academy joins the nav, after Laws.** A seventh left-hand item does not
  fit the untightened header at any width below 1440px, so the tightened
  rule in chrome.css now applies up to 1440px (was 1280px), the base tracking
  is .10em and the group gap 20px (were .12em and 24px) so the seven items
  centre the mark from 1440px up, and the left group stands down at 1000px
  (was 940px), with the 1000px to 1134px band letting the mark sit a few
  pixels off centre as the comment always allowed. Measured, not guessed:
  the numbers are in the comment beside each rule, with the three old values
  and the one header line to remove if the item ever comes out again. The
  header test's expected list and current-page map carry the new item; the
  footer's Explore list, ten curated search rows with anchors into the
  lessons, and a line on ask.html point at the page.
