...
```
      ...
      │   caretaker/*    Caretaker Card programme (apply, card, shipping)    │
...
| `caretaker*` (7 collections) | caretaker routes | admin, public card page | applications, cards, addresses, shipments; public projection cannot leak the holder (tested) |
...
- Legacy shared-secret fallbacks (`PFA_ADMIN_TOKEN`, `PFA_ADMIN_API_KEY`)
  still exist for transition and use constant-time comparison; **remove those
  env vars once Firebase login is confirmed** (Handbook §2b).
...
- Default deny. Browsers can read only a member's own record (signed in) and
  the Circle's public content. Every `caretaker*`, `storeOrders`, `submissions`
...
- **Payload budget**: `index.html` ≈ 57 KB, `store.html` ≈ 119 KB, other pages
  ...
  if a page grows.
- **Cron**: one daily job (`/api/caretaker/email-worker`, 03:00 UTC) — Hobby
...
`PFA_ADMIN_TOKEN` `PFA_ADMIN_API_KEY` (legacy — remove)
