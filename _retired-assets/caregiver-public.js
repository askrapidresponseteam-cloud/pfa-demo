...
(function () {
  ...
  var held = null;
  try { held = JSON.parse(localStorage.getItem('pfa_caregiver') || 'null'); } catch (_) { held = null; }
  ...
  function fail(message) {
    host.innerHTML = '<div class="empty-state"><strong>' + esc(message)
      + '</strong><a class="btn dark" href="caregiver.html">Apply for a card</a></div>';
  }
  ...
  function render(card) {
    ...
    host.innerHTML = '<section class="order-success">'
      ...
      + '<p class="kicker">Colony Animal Colony Caregiver Card</p>'
      ...
      + (isHolder
        ...
        : '<div class="form-shell"><div class="form-head"><h3>Verified</h3>'
          ...
          + '</div><div class="form-actions"><a class="btn dark block" href="caregiver.html">Get your own card</a></div>'
          + '</div></div>')
      + '</div>'
      + '<div>'
      + (isHolder
  ...
  fetch('/api/caregiver/card?id=' + encodeURIComponent(cardId), { headers: { Accept: 'application/json' } })
