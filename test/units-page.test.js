...
test('the hero starts where every other section hero starts', () => {
  ...
  /* newsroom.html left this list on 16 Sep 2026: it opens on its lead
     story now, a full-bleed photograph behind a transparent header, so it
     is no longer a compact section hero and cannot be the reference for one. */
  for (const page of ['laws.html', 'get-involved.html']) {
...
test('the plot of dots is gone, and nothing draws a map', () => {
  /* Twice tried, twice removed, same verdict both times. The hero and the
     sidebar each carried a scatter of every unit; it read as dots rather
     than as a country and was removed. v1.303 tried again as a full band
     with every unit joined to its nearest neighbour, and the owner's
     review was shorter than this comment. Eighty points do not draw
     India, with or without a web, and no third attempt should reach a
     review: the numbers, the list and the search are the page. */
  assert.equal(d.querySelectorAll('.pt').length, 0);
  assert.ok(!/u-atlas|heroAtlas|sideAtlas|u-plot|u-dot|u-web|uplot/.test(HTML),
    'the plot machinery, of either generation, must stay gone');
