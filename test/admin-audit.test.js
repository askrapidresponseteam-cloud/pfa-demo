...
test('a log that cannot be written does not break the action it describes', async () => {
  ...
  const entry = await audit.record(WHO, { module: 'cards', action: 'card-batch', subject: 'B-12' }, { headers: {} },
    { getDb: () => broken, now: () => 1750000000000 });
  assert.equal(entry.action, 'card-batch', 'it resolves rather than throwing');

  const rejecting = { collection: () => ({ doc: () => ({ create: () => Promise.reject(new Error('permission denied')) }) }) };
  await audit.record(WHO, { module: 'cards', action: 'card-batch', subject: 'B-13' }, { headers: {} },
...
test('every admin route that changes something writes to the log', () => {
  ...
  for (const file of ['case.js', 'people.js', 'cards.js', 'submission-status.js']) {
