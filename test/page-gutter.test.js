...
test('the gutter is no longer a flat phone value on a laptop', () => {
  const token = mainGutter(fs.readFileSync(path.join(ROOT, 'quiz.html'), 'utf8'));
...
test('the generated pages carry the same token as the shop they are built from', () => {
  const shop = mainGutter(fs.readFileSync(path.join(ROOT, 'quiz.html'), 'utf8'));
  for (const page of ['track.html']) {
