...
/* The JavaScript written inside the pages, linted.
   ...
   JavaScript - admin.html's panel, quiz.html's whole quiz, wall.html's
   ...
   is a credit to the pages, and one of them was real: a page declared
...
test('the check is looking at the pages that carry the most of it', () => {
  ...
  /* The floor was 250KB. It is 200 because the theatre, 66KB of it, left
     wall.html for assets/theatre.js, which eslint covers directly as an
     ordinary file. The number guards against the regex silently ceasing
     to match, not against a page being relieved of script on purpose. */
  assert.ok(total > 200 * 1024, `only ${Math.round(total / 1024)} KB of inline script was found; the extractor has stopped matching`);
  /* wall.html is not on this list any more: its player moved to
     assets/theatre.js, which `npm run lint` covers as an ordinary file.
     Leaving it here would demand that a page keep carrying inline script
     it has been deliberately relieved of. Nor is founder.html, for the same
     reason: it mounts that module now instead of carrying a player. */
  for (const page of ['admin.html', 'quiz.html', 'units.html', 'donate.html']) {
