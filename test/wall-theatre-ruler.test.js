...
/* The ruler belongs to the player, and the player is assets/theatre.js now,
   shared with the founder page. Read where it lives. */
const wall = fs.readFileSync(path.join(ROOT, 'assets', 'theatre.js'), 'utf8');
/* The film list and what a film may declare stayed on the page: they are the
   wall's content, not the player's machinery. */
const page = fs.readFileSync(path.join(ROOT, 'wall.html'), 'utf8');
...
test('a film may say where to start, and the theatre passes it to the player', () => {
  /* A film may carry a start point. The sentence documenting it left with
     the film list; what matters is that the player still honours it. */
  assert.match(wall, /it\.start/, 'the player reads a start point off the film');
  assert.match(wall, /\(at \? '&start=' \+ at : ''\)/, 'YouTube');
  assert.match(wall, /\(at \? '#t=' \+ at \+ 's' : ''\)/, 'Vimeo');
  /* The films left wall.html: they come from /api/wall now, and the page
     ships with none. What is asserted here is that a start point still
     survives the journey, so the fixture the theatre tests drive is where
     it is checked. */
  const fixture = fs.readFileSync(path.join(ROOT, 'test', 'wall-theatre.test.js'), 'utf8');
  const film = /\{wall:'long', title:'([^']+)', credit:'([^']+)', yt:'BVbpSc_EXlI', start:3753\}/.exec(fixture);
