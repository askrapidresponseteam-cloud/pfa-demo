...
test('a page that asks for Marcellus has actually declared it', () => {
  /* enforce.html shipped naming Marcellus in --display with no @font-face and
     ...
     all is skipped by exactly the same line. So the question here is the
     other way round - not "having asked for the file, is the page safe" but
     "having asked for the face, did the page ask for anything at all". */
  const missing = [];
  for (const page of pages) {
    const html = fs.readFileSync(path.join(ROOT, page), 'utf8');
    const stack = /--display:\s*Marcellus\b/.test(html);
    if (!stack) continue;
    /* The two matched faces are the floor: they are what admin.html carries
       instead of the web font, and what keeps a cold load from reflowing. */
    for (const face of Object.keys(OVERRIDES)) {
      if (!html.includes(`font-family: ${face}`)) missing.push(`${page}: names Marcellus but declares no ${face}`);
    }
    /* Matched whitespace-loosely on purpose. The test above writes the same
       check with a space after `src:`, which is the only reason admin.html
       was ever skipped by it: the panel is minified, its rule reads
       `src:url(`, and a page that does load the face was being read as one
       that does not. */
    if (!/src:\s*url\("?\/?fonts\/marcellus-latin\.woff2"?\)/.test(html)) {
      missing.push(`${page}: names Marcellus but never loads fonts/marcellus-latin.woff2`);
    }
  }
