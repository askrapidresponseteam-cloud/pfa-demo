...
/* The claim being tested is "no field on either card is ever blank". That is a
   ...
   Six inputs on the Patron card and seven on the Caregiver card, so 64 and 128
...
test('no Caregiver card field is ever blank, for any combination of missing input', () => {
  const full = {
  ...
  for (const withheld of subsets(keys)) {
    ...
    const out = F.caretaker(input, NOW);
    for (const [field, value] of caretakerStrings(out)) {
      assert.equal(typeof value, 'string', `${field} is not a string without [${withheld}]`);
      assert.ok(value.trim().length > 0, `${field} is blank when [${withheld}] are missing`);
      checked += 1;
    }
...
test('empty strings, whitespace and nulls count as missing, not as values', () => {
  ...
  for (const empty of ['', '   ', null, undefined, '\n\t ']) {
    ...
    const c = F.caretaker({ cardId: 'PFA-CCT-4K7M2QX9', address: empty, mobile: empty, email: empty }, NOW);
...
test('the QR always has something valid to encode', () => {
  ...
  assert.ok(F.caretaker({ cardId: 'PFA-CCT-4K7M2QX9' }, NOW).qr.includes('PFA-CCT-4K7M2QX9'));
  ...
  const noNumber = F.caretaker({}, NOW);
...
test('every page that draws a card also loads the field completer', () => {
  ...
  const drawing = pages.filter((f) => {
    ...
    return /assets\/(patron-card-pdf|caretaker-card)\.js/.test(html);
  });

  assert.ok(drawing.length >= 4, `expected several card pages, found ${drawing.length}`);
