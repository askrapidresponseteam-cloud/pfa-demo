...
test('the index the search box loads carries no private row', () => {
  /* One file now, { pages, rows }, written by scripts/build-search-index.js
     at the root where the client fetches it. Both layers are checked: pages
     is what the tests and sitemap read, rows is what the engine serves. */
  const index = JSON.parse(read('search-index.json'));
  assert.ok(Array.isArray(index.rows) && index.rows.length > 300, 'the rows layer is missing or truncated');
  const leaked = index.rows.filter((r) => isPrivatePath(r && r.u)).map((r) => r.u)
    .concat(index.pages.filter((p) => isPrivatePath(p.url)).map((p) => p.url));
