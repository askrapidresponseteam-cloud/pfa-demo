...
test('a double press or a retry after a lost answer is one submission with one number', async () => {
  const db = fakeDb();
  const handler = handlerWith(db);
  const body = { kind: 'PFA-Q', data: { question: 'Where do I take a hurt crow?', topic: 'Something else', state: 'Karnataka', city: 'Udupi', name: 'Asha Rao', email: 'asha@example.com', message: 'Where do I take a hurt crow?' }, clientRequestId: 'k-1' };
