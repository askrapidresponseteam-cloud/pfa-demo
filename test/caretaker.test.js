...
test('identifiers are distinguishable and unambiguous to read aloud', () => {
  assert.match(CARETAKER.createCardId(), CARETAKER.CARD_ID_PATTERN);
  assert.match(CARETAKER.createOrderId(), CARETAKER.ORDER_ID_PATTERN);
  assert.match(CARETAKER.createShipmentId(), CARETAKER.SHIPMENT_ID_PATTERN);
  assert.ok(!/[01IO]/.test(CARETAKER.ALPHABET), 'ambiguous glyphs must not appear in card numbers');
});

test('the application asks for five things and validates every one', () => {
  const parsed = CARETAKER.parseApplication(application);
  ...
  assert.throws(() => CARETAKER.parseApplication({ ...application, mobile: '1234567890' }), /mobile/i);
  assert.throws(() => CARETAKER.parseApplication({ ...application, email: 'not-an-email' }), /email/i);
  assert.throws(() => CARETAKER.parseApplication({ ...application, name: 'A' }), /name/i);
  assert.throws(() => CARETAKER.parseApplication({ ...application, address: 'C-25, Shanti Nagar' }), /PIN/i);
});
...
test('mobile numbers normalise to one shape so duplicates cannot slip past', () => {
  ['9876543210', '+91 98765 43210', '09876543210', '91-9876543210'].forEach((input) => {
    assert.equal(CARETAKER.normaliseMobile(input), '9876543210');
  });
});

test('delivery defaults to the card address and is only asked for when it differs', () => {
  assert.deepEqual(CARETAKER.parseDeliveryChoice({}), { sameAsCardAddress: true });
  assert.deepEqual(CARETAKER.parseDeliveryChoice({ deliverElsewhere: 'no' }), { sameAsCardAddress: true });

  const elsewhere = CARETAKER.parseDeliveryChoice({
  ...
  assert.throws(() => CARETAKER.parseDeliveryChoice({ deliverElsewhere: 'yes', recipient: 'X' }), /delivery address/i);
});

test('card validity runs three years from issue', () => {
  const { issuedAt, validUntil } = CARETAKER.computeValidity('2026-08-21T00:00:00.000Z');
  assert.equal(new Date(validUntil).getUTCFullYear() - new Date(issuedAt).getUTCFullYear(), 3);
  assert.equal(CARETAKER.cardStanding({ status: 'active', validUntil }, '2027-01-01'), 'active');
  assert.equal(CARETAKER.cardStanding({ status: 'active', validUntil }, '2030-01-01'), 'expired');
  assert.equal(CARETAKER.cardStanding({ status: 'revoked', validUntil }, '2027-01-01'), 'revoked');
});

test('a shipment can only move forwards, and never out of a terminal state', () => {
  assert.ok(CARETAKER.canTransition(null, 'order_confirmed'));
  assert.ok(CARETAKER.canTransition('order_confirmed', 'preparing'));
  assert.ok(CARETAKER.canTransition('preparing', 'out_for_delivery'), 'skipping ahead is allowed');

  assert.ok(!CARETAKER.canTransition('dispatched', 'preparing'), 'no walking backwards');
  assert.ok(!CARETAKER.canTransition('delivered', 'in_transit'), 'delivered is final');
  assert.ok(!CARETAKER.canTransition('cancelled', 'dispatched'), 'cancelled is final');
  assert.ok(!CARETAKER.canTransition('preparing', 'preparing'), 'no self-transitions');
  assert.ok(!CARETAKER.canTransition('preparing', 'teleported'), 'unknown states are rejected');

  assert.ok(CARETAKER.canTransition('in_transit', 'exception'), 'exits are always reachable');
  assert.ok(CARETAKER.canTransition('exception', 'out_for_delivery'), 'an exception can be recovered');
  assert.ok(!CARETAKER.canTransition('exception', 'preparing'), 'but not back to before dispatch');
});

test('only meaningful delivery steps are emailed', () => {
  assert.ok(CARETAKER.shouldNotify('dispatched'));
  assert.ok(CARETAKER.shouldNotify('delivered'));
  assert.ok(!CARETAKER.shouldNotify('in_transit'), 'four courier scans must not send four emails');
  assert.ok(!CARETAKER.shouldNotify('preparing'));
});

test('the public projection cannot leak the holder', () => {
  const projection = CARETAKER.publicProjection({
    card: {
...
test('card tokens are stored hashed and compared in constant time', () => {
  const raw = CARETAKER.createCardToken();
  const stored = CARETAKER.hashToken(raw);
  assert.notEqual(stored, raw);
  assert.ok(CARETAKER.safeEqual(stored, CARETAKER.hashToken(raw)));
  assert.ok(!CARETAKER.safeEqual(stored, CARETAKER.hashToken(CARETAKER.createCardToken())));
  assert.ok(!CARETAKER.safeEqual(stored, 'short'), 'length mismatch must not throw');
});

test('caretaker shipping cannot be pushed through the generic payment endpoint', () => {
  assert.throws(() => parsePaymentRequest({ type: 'caretaker', amount: '1' }), /card page/i);
});
...
function loadRenderer() {
  const source = fs.readFileSync(path.join(__dirname, '..', 'assets', 'caretaker-card.js'), 'utf8');
  const context = {
  ...
  return context.window.PFACaretakerCard;
}
...
test('identity fingerprints catch the same person applying twice', () => {
  const asha = CARETAKER.identityKey('Asha Kumar', '110031');
  assert.equal(CARETAKER.identityKey('Dr. Asha Kumar', '110031'), asha, 'honorifics are stripped');
  assert.equal(CARETAKER.identityKey('  asha   KUMAR ', '110031'), asha, 'case and spacing are normalised');
  assert.equal(CARETAKER.identityKey('Kumar Asha', '110031'), asha, 'name order does not matter');
  assert.notEqual(CARETAKER.identityKey('Asha Kumar', '560001'), asha, 'a different PIN is a different person');
  assert.notEqual(CARETAKER.identityKey('Meena Kumar', '110031'), asha, 'a different name is a different person');
});

test('a household key is stable for one address and unique across addresses', () => {
  const a = CARETAKER.householdKey('C-25, Shanti Nagar, Block A', '110031');
  assert.equal(CARETAKER.householdKey('c-25  shanti nagar block a', '110031'), a);
  assert.notEqual(CARETAKER.householdKey('C-26, Shanti Nagar, Block A', '110031'), a);
});

test('the printed Patron card is mandatory and cannot be declined by the client', () => {
  const base = {
    type: 'membership', name: 'Asha Kumar', mobile: '9876543210', email: 'a@b.com',
    address: 'C-25 Shanti Nagar', pin: '110031', district: 'Delhi', state: 'Delhi'
  };
  assert.equal(parsePaymentRequest(base).amount, '514.00');
  assert.equal(parsePaymentRequest({ ...base, physicalCard: 'no' }).amount, '514.00',
    'a browser claiming no printed card must not reduce the amount');
  assert.equal(parsePaymentRequest({ ...base, physicalCard: 'no' }).metadata.physicalCard, true);

  const usd = parsePaymentRequest({ ...base, currency: 'usd' });
  assert.equal(usd.metadata.physicalCard, false, 'PFA does not post internationally');
});
...
test('the chairperson signature only appears on a card that has actually been issued', async () => {
  ...
  vm.runInContext(source, context);
  const Card = context.window.PFACaretakerCard;
...
test('the card prints the issue date and no expiry', () => {
  ...
  assert.ok(source.includes("var ROLE = 'Colony Caregiver'"), 'the role reads Caregiver');
});
