...
test('the theatre is exempt, and stays exempt', () => {
  /* The player is assets/theatre.{css,js} now, shared with the founder page,
     so its rule is proved where the rule lives. */
  const wall = fs.readFileSync(path.join(ROOT, 'assets', 'theatre.css'), 'utf8');
...
test('the theatre clock and seek bar are live only when there is a duration to measure against', () => {
  ...
  /* The player moved to assets/theatre.js, shared with the founder page, so
   what it promises is proved where the promise now lives. */
const src = fs.readFileSync(path.join(__dirname, '..', 'assets', 'theatre.js'), 'utf8');
