...
| `PFA_MAIL_REPLY_TO` | Where replies to confirmations go. Every confirmation says "reply to this email", so point it at an inbox someone reads. |
...
| `PFA_ADMIN_TOKEN` | Trigger for the caregiver email worker only. It no longer opens any admin route. |
| `PFA_ADMIN_API_KEY` | Retired. Nothing reads it; remove it from the environment. |
...
- **Caregivers** - the `caretakerPublic` collection, kept separate.
...
**About the two older secrets.** `PFA_ADMIN_API_KEY` and `PFA_ADMIN_TOKEN` were
single shared passwords with no named user and no audit trail. Both have been
retired from the admin guard: the routes now accept only real admin accounts.
Delete `PFA_ADMIN_API_KEY` from the environment - nothing reads it. Keep
`PFA_ADMIN_TOKEN` only if you run the caregiver email worker (§6), which is the
one thing still using it, and note that holding it no longer grants any admin
access.
...
Every submission and payment is confirmed by email (see `lib/confirmations.js`), and each page tells the sender to check Spam and Junk. How often they have to depends on the sending domain: in Resend, verify `peopleforanimalsindia.org` and add the SPF, DKIM and DMARC records it lists, and send from that domain only. A confirmation the provider cannot take at once waits on the `caregiverEmails` queue for the daily email worker.
...
- **Card artwork on the public card URL.** `caregiver-card.html?id=…` exists;
...
| Payments | `transactions` | Membership and colony caregiver card payments through CCAvenue |
