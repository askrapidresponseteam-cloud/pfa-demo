...
const memory = { orders: new Map(), intents: new Map(), events: new Map() };
...
function applyEvent(existing, topic, payload) {
  ...
  switch (topic) {
    ...
    case 'orders/paid': {
      ...
      /* Nor take a paid order back to unpaid. orders/create and orders/paid
         arrive in whichever order Shopify sends them, and a create generated
         while the payment was still pending carries financial_status
         "pending" - which reads as AWAITING_PAYMENT. Arriving second, it used
         to overwrite CONFIRMED, and the shopper polling their confirmation
         watched a paid order say it was waiting for payment. A create can
         still move an order forward; it can no longer move it back. */
      const paidAlready = existing && existing.status === 'CONFIRMED'
        && (created.status === 'AWAITING_PAYMENT' || created.status === 'PAYMENT_FAILED');
      const status = existing && terminal.includes(existing.status) ? existing.status
        : paidAlready ? existing.status : created.status;
    ...
    case 'refunds/create': {
      ...
      /* The only handler that adds rather than recomputes, so it is the only
         one a replay can double. Shopify's own id decides: the same refund
         seen twice updates its row, it does not make a second one, and
         refundedTotal cannot drift above what was actually refunded. */
      const refundId = cleanText(payload.id, 30);
      const row = { refundId, amount, at: cleanText(payload.created_at, 40) || now };
      const seen = refundId ? refunds.findIndex((r) => r && r.refundId === refundId) : -1;
      if (seen > -1) refunds[seen] = row; else refunds.push(row);
...
/* A marker is a claim first and a receipt second.

   It used to be only a claim: written before the record, so that two
   simultaneous deliveries could not both apply - but a write that then failed
   left the claim standing, and Shopify's retry found it, was answered
   "duplicate", and the event was lost for good. A paid order nobody was ever
   told about is what the nightly reconciler exists to catch, and exactly what
   it should not have to.

   So the marker carries `applied`. Only a marker that says the work landed
   turns a redelivery away; one left behind by a failed attempt lets the retry
   through. Two deliveries racing can then both apply, which is harmless:
   applyEvent recomputes the record from the payload it was given, and
   refunds/create - the one handler that adds rather than recomputes - keys on
   Shopify's own refund id. */
async function markEventSeen(webhookId) {
  if (!webhookId) return false; // no id → cannot dedupe, process anyway
  if (!firebaseConfigured()) {
    if (memory.events.get(webhookId) === true) return true;
    memory.events.set(webhookId, false);
    return false;
  }
  const ref = db().collection('storeWebhookEvents').doc(cleanText(webhookId, 120));
  try {
    await ref.create({ receivedAt: new Date().toISOString(), applied: false });
    return false;
  } catch (error) {
    if (error && (error.code === 6 || /already exists/i.test(String(error.message)))) {
      const seen = await ref.get();
      return Boolean(seen.exists && (seen.data() || {}).applied === true);
    }
    throw error;
  }
}

/* The receipt half, written once the record is safely stored. */
async function markEventApplied(webhookId) {
  if (!webhookId) return;
  try {
    if (!firebaseConfigured()) { memory.events.set(webhookId, true); return; }
    await db().collection('storeWebhookEvents').doc(cleanText(webhookId, 120))
      .set({ applied: true, appliedAt: new Date().toISOString() }, { merge: true });
  } catch (error) {
    /* The record is stored; failing here only risks the event being applied a
       second time, which recomputes the same state. Never worth a 5xx. */
    console.warn('webhook marker not completed', { webhookId, message: error && error.message });
  }
}

async function upsertFromWebhook(topic, payload, webhookId) {
  if (await markEventSeen(webhookId)) return { duplicate: true };
  const result = await applyWebhook(topic, payload, webhookId);
  await markEventApplied(webhookId);
  return result;
}

async function applyWebhook(topic, payload, webhookId) {
...
module.exports = {
  ...
  _private: { memory, markEventSeen, markEventApplied, applyEvent }
};
