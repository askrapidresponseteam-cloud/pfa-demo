...
const TEMPLATES = {
  ...
  patron_card(payload) {
    return {
      subject: `Your PFA Patron Card - ${payload.memberId}`,
      html: shell({
        heading: 'Your Patron Card is ready.',
        intro: `${payload.name ? payload.name + ', y' : 'Y'}our People for Animals Patron Card is issued in your name. `
          + 'Your member number is below; it is how the card is verified and how you sign in to the Members area, where the card can be downloaded or shown from your phone.',
        rows: [
          { label: 'Member number', value: payload.memberId },
          { label: 'Member since', value: formatDate(payload.memberSince) },
          { label: 'Valid until', value: formatDate(payload.validUntil) }
        ],
        cta: { label: 'Open the Members area', url: payload.memberUrl },
        footnote: 'Signing in asks for your member number and a one-time code sent to this address. Nobody at PFA will ever ask you for that code.'
      }),
      text: textFrom([
        `${payload.name ? payload.name + ', y' : 'Y'}our People for Animals Patron Card is issued.`,
        `Member number: ${payload.memberId}`,
        `Member since: ${formatDate(payload.memberSince)}`,
        `Valid until: ${formatDate(payload.validUntil)}`,
        `Open the Members area: ${payload.memberUrl}`,
        'Signing in asks for your member number and a one-time code sent to this address.'
      ])
    };
  },
