...
// Colony Animal Colony Caregiver Card numbers. The same format is used whether the
...
function createCaregiverCardId() {
...
</style></head><body><main class="wrap"><section class="card"><img class="logo" src="/img/logo.png" alt="People for Animals"><h1>${escapeHtml(title)}</h1><p>${escapeHtml(message)}</p><a class="btn" href="${escapeHtml(backUrl)}">${escapeHtml(backLabel)}</a></section></main></body></html>`);
}
...
function renderTransfer(response, merchantValues, options) {
  ...
  const returnUrl = cleanText(options && options.returnUrl || '/', 200);
  const orderId = cleanText(merchantValues && merchantValues.order_id || '', 80);
...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escapeHtml(title)} | PFA</title><style>
*{box-sizing:border-box}body{margin:0;background:#fff;color:#111;font-family:Arial,sans-serif}.wrap{min-height:100vh;display:grid;place-items:center;padding:24px}.card{width:min(560px,100%);border:1px solid #ddd;padding:34px;text-align:center}.logo{width:180px;max-width:55%;height:auto;margin-bottom:30px}h1{font-size:30px;line-height:1.1;margin:0 0 12px}p{font-size:16px;line-height:1.55;color:#555}.btn{border:0;background:#111;color:#fff;padding:14px 20px;font:700 16px Arial,sans-serif;cursor:pointer;margin-top:16px}.leave{display:inline-block;margin-top:18px;font-size:13px;color:#555;text-decoration:underline}.spinner{width:30px;height:30px;border:3px solid #ddd;border-top-color:#111;border-radius:50%;margin:22px auto;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
</style></head><body><main class="wrap"><section class="card"><img class="logo" src="/img/logo.png" alt="People for Animals"><h1>${escapeHtml(title)}</h1><div class="spinner" aria-hidden="true"></div><p>${escapeHtml(message)}</p><form id="ccaForm" method="post" action="${escapeHtml(paymentUrl)}"><input type="hidden" name="encRequest" value="${escapeHtml(encryptedRequest)}"><input type="hidden" name="access_code" value="${escapeHtml(accessCode)}"><button class="btn" type="submit">Continue securely</button></form><a class="leave" href="${escapeHtml(returnUrl)}">Cancel and go back \u2014 no payment is taken</a></section></main><script nonce="${nonce}">
/* The hand-off runs once. Coming BACK to this page - the Back button from
   CCAvenue, or their close button landing the visitor here - must never
   fire the transfer again, or every way out of the checkout loops straight
   back into it and the close button reads as broken. On a return visit the
   spinner stops and the page waits: Continue securely to try again, or the
   cancel link home. */
(function(){
  var KEY = 'pfa:transfer:${escapeHtml(orderId)}';
  var spin = document.querySelector('.spinner');
  function arm(again){
    if (spin) spin.style.display = again ? 'none' : '';
  }
  function returned(){
  ...
  window.addEventListener('pageshow', function(e){
    if (e.persisted){ arm(true); return; }   /* restored from bfcache: wait for the visitor */
    if (returned()){ arm(true); return; }    /* a fresh load of a page already used once */
    mark(); arm(false);
    document.getElementById('ccaForm').submit();
  });
})();
</script></body></html>`);
}
