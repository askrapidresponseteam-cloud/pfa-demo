    ...
    if (wantsList) {
      data = {
        ...
        products: data.products.map((p) => ({
          ...
          variants: (p.variants || []).map((v) => ({ id: v.id, title: v.title, sku: v.sku, available: v.available, stock: v.stock, price: v.price, compareAtPrice: v.compareAtPrice, image: v.image ? { src: v.image.src } : null, options: v.options }))
        }))
      };
    }
