...
function toFilm(doc) {
  const d = doc.data() || {};
  /* The intake writes the sender's fields under `fields` (lib/routes/
     pfa-submissions.js); this read `data`, so a published film never had a
     url to read and the wall stayed empty however many were approved.
     Both keys are read, the stored one first. */
  const given = d.fields || d.data || {};
