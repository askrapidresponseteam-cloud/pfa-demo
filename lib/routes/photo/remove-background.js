...
/* This route spends money. Every call is a paid request to the provider, it
   needs no sign-in, and the payload the caller chooses is what gets paid for -
   so without a brake a loop here is somebody else's bill. Twelve photographs
   in a quarter of an hour is more than anyone preparing one card will use.
   Per warm instance, like the other counters in this project: a brake, not a
   wall, and the size cap and the byte check stand behind it. */
const WINDOW_MS = 15 * 60 * 1000;
const LIMIT = 12;
const calls = new Map();

function tooMany(ip, nowMs) {
  const key = String(ip || 'unknown');
  const entry = calls.get(key);
  if (!entry || entry.resetAt <= nowMs) {
    calls.set(key, { count: 1, resetAt: nowMs + WINDOW_MS });
...
module.exports = async function handler(request, response) {
  ...
  if (tooMany(S.clientIp(request), Date.now())) {
    return sendJson(response, 429, { error: 'That is a lot of photographs at once. Wait a few minutes and try again.' });
  }
  ...
  /* What the bytes say they are, not what the data URL claimed. Anything else
     is not a photograph, and forwarding it would buy the sender a paid call to
     a segmentation model with whatever they liked in it. */
  if (!S.imageType(bytes)) return sendJson(response, 415, { error: 'Send a JPEG, PNG or WebP photograph.' });
...
module.exports._private = { tooMany, LIMIT, resetForTests: () => calls.clear() };
