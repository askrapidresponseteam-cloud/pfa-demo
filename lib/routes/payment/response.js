...
async function sendReceipt(orderId, transaction, callback, firstSuccess, siteUrl) {
  ...
  if (!to || !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(to)) return { sent: false, reason: 'NO_EMAIL' };
  if (!caregiverMail.isConfigured()) return { sent: false, reason: 'MAIL_NOT_CONFIGURED' };
  ...
  if (transaction.receiptSentAt) return { sent: false, reason: 'ALREADY_SENT' };
  if (!firstSuccess) {
    const fresh = await getTransaction(orderId);
    if (fresh && fresh.receiptSentAt) return { sent: false, reason: 'ALREADY_SENT' };
  }
  const metadata = transaction.metadata || {};
  ...
  const template = transaction.type === 'membership' ? 'membership_welcome' : 'payment_received';
  const sent = await Promise.race([
    caregiverMail.deliver({ to, template, payload }),
    new Promise((_, reject) => setTimeout(() => reject(new Error('receipt timed out')), RECEIPT_TIMEOUT_MS))
  ]);
  await getDb().collection('transactions').doc(orderId).update({
    receiptSentAt: new Date().toISOString(),
    receiptProviderId: cleanText(sent && sent.providerId, 120) || null
  });
  return { sent: true };
}
...
function renderMembershipWelcome(response, baseUrl, transaction, result, memberRef, receiptTo) {
  const customer = transaction.customer || {};
  const metadata = transaction.metadata || {};
  ...
  const letter = receiptTo
    ? `Your welcome letter is on its way to <strong>${escapeHtml(receiptTo)}</strong>.`
    : 'Keep this number: it is your membership. A welcome letter could not be sent just now, and this page is your record until it can.';
...
function renderResult(response, baseUrl, transaction, result, memberId, caregiver, applicationRef, receiptTo) {
  ...
  const message = success
    ? transaction.type === 'membership'
      ? 'Welcome to People for Animals. Keep the member number below: it is yours from today, and your card and kit follow in 20 to 25 days.' + (receiptTo ? ` Your welcome letter has been emailed to ${receiptTo}.` : '')
    ...
    : transaction.type === 'caregiver'
      ...
      : 'Thank you for supporting People for Animals. Keep the PFA transaction ID for your records.'
          + (receiptTo ? ` A receipt has been emailed to ${receiptTo}.` : '')
    : pending
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    /* The donor's receipt. Sent once, on the callback that made the payment
       succeed, and recorded on the transaction so a redelivered callback or a
       refreshed page never sends a second. A slow or unset mail provider is
       not allowed to hold up the page the person is waiting on. */
    let receiptTo = '';
    /* a donation's receipt, or a member's welcome letter: sendReceipt picks the template */
    if ((transaction.type === 'donate' || transaction.type === 'membership') && effective === 'success') {
      try {
        const receipt = await sendReceipt(orderId, { ...transaction, ...updated, membershipReference: applicationRef || transaction.membershipReference }, callback, updated.firstSuccess === true, baseUrl);
        if (receipt.sent || receipt.reason === 'ALREADY_SENT') receiptTo = cleanText((transaction.customer || {}).email, 160);
      } catch (mailError) {
    ...
    if (transaction.type === 'membership' && effective === 'success') {
      return renderMembershipWelcome(response, baseUrl, { ...transaction, ...updated }, { ...callback, status: effective }, applicationRef, receiptTo);
    }
    return renderResult(
      response,
      baseUrl,
