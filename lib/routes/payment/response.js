...
async function sendReceipt(orderId, transaction, callback, firstSuccess) {
  const customer = transaction.customer || {};
  ...
  const sent = await Promise.race([
    caregiverMail.deliver({ to, template: 'payment_received', payload }),
...
function renderResult(response, baseUrl, transaction, result, memberId, caregiver, applicationRef, receiptTo) {
  ...
  const action = `<a class="btn dark" href="${escapeHtml(baseUrl)}/${isCaregiver ? 'get-involved.html#caregiver' : 'donate.html'}">Return to PFA</a>`;
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (!/^PFA-(?:DON|SND|CAR|CGA)-[A-Z0-9]{8}$/.test(orderId)) throw new Error('The PFA transaction ID is invalid.');
    const transaction = await getTransaction(orderId);
    if (!transaction || !['donate', 'send', 'caregiver', 'caregiver-application'].includes(transaction.type)) throw new Error('The stored PFA transaction could not be found.');
    ...
    let receiptTo = '';
    if ((transaction.type === 'donate' || transaction.type === 'send') && effective === 'success') {
      try {
        const receipt = await sendReceipt(orderId, { ...transaction, ...updated }, callback, updated.firstSuccess === true);
