...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    cardId = CAREGIVER.clean(parsed.searchParams.get('id'), 60).toUpperCase();
  } catch (_) {
    cardId = '';
  }
  ...
  try {
    ...
    if (!card) {
      ...
      return sendJson(response, 404,
        { code: 'NOT_FOUND', message: 'No Colony Caregiver Card was found for that number.' },
    ...
    console.error('PFA colony caregiver card read error:', CAREGIVER.clean(error && error.message, 240));
