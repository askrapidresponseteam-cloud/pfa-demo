...
function createHandler(deps) {
  ...
  async function receive(request, response) {
    ...
    /* The lookup path has always been braked; sending never was, so a loop
       could allocate references and write documents without limit. */
    if (S.writeLimited(S.clientIp(request), now())) {
      return fail(429, 'That is a lot of forms from one connection in a short time. Wait a few minutes and try again. If an animal is in danger now, call 112.');
    }
    ...
    if (dedupe) {
      ...
      if (seen.exists && seen.data() && seen.data().reference) {
        ...
        /* The page is told again what it was told the first time. */
        const told = earlier.confirmation || { state: earlier.acknowledged ? 'sent' : 'none', to: '' };
        const repeated = CONFIRM.notice({ state: told.state, to: told.to, number: CONFIRM.forKind(kind).number, reference: earlier.reference });
        if (form) {
        ...
        return sendJson(response, 200, {
          ...
          attachments: Number(earlier.attachments) || 0, duplicate: true, confirmation: repeated
        });
      }
    }
    ...
    const outcome = await CONFIRM.send({
      ...
      dedupeKey: `submission_received:${reference}`,
      payload: {
        kind, name: nameIn(clean), reference, kindLabel: S.KIND_LABELS[kind], receivedAt: createdAt,
        siteUrl: siteUrl(request),
        followUrl: `${siteUrl(request)}/track.html#ref=${encodeURIComponent(reference)}`
      },
      mail: { deliver, isConfigured },
      queue,
      timeoutMs: ACK_TIMEOUT_MS
    });
    const acknowledged = outcome.state === 'sent';
    const confirmation = CONFIRM.notice({ state: outcome.state, to: outcome.to, number: CONFIRM.forKind(kind).number, reference });
    ...
    if (dedupe) {
      ...
      try {
        await dedupe.create({
          reference, kind, createdAt, acknowledged, attachments: photos.accepted.length,
          confirmation: { state: outcome.state, to: outcome.to }
        });
      } catch (error) {
    ...
    return sendJson(response, 200, {
      ok: true, reference, kindLabel: S.KIND_LABELS[kind], receivedAt: createdAt,
      acknowledged, attachments: photos.accepted.length, confirmation
    });
  }
