...
const enforceCounter = require('../enforce-counter');
...
function createHandler(deps) {
  ...
  const fieldValue = deps.fieldValue || firebase.fieldValue;
  ...
  async function receive(request, response) {
    ...
    /* The public register on enforce.html is a count, and enforce-counter.js
       keeps it: one field on one document, so the dashboard reads a single
       document rather than every cruelty report ever filed. It is a separate
       module because the increment is a merge, and nothing that merges belongs
       in the file that writes submissions. Never allowed to fail the response. */
    if (kind === 'PFA-CR' && clean.state) {
      await enforceCounter.bump(db, { state: clean.state, stage: clean.policeStage, at: createdAt, fieldValue });
    }
