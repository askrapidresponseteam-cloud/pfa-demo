    ...
    try {
      ...
      if (action === 'wall') {
        ...
        if (data.kind !== 'PFA-S' && data.kind !== 'PFA-W') {
          return sendJson(response, 400, { code: 'NOT_A_FILM', message: 'Only a wall submission or a newsroom field note can be published.' });
        }
        const published = body.published === true || body.published === 'true';
        await ref.set({
          wall: { published, by, at },
          history: fieldValue().arrayUnion({ status: current, event: published ? 'wall-publish' : 'wall-remove', at, by }),
          updatedAt: at
        }, { merge: true });
        audit.record(who, { module: 'submissions', action: 'wall', subject: reference,
          detail: published ? 'Put on the wall' : 'Taken off the wall' }, request);
        return sendJson(response, 200, { ok: true, action, published, at, by });
      }
