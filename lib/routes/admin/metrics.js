...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    const [
      ...
      storeOrdersTotal, storeOrdersOpen
    ] = await Promise.all([
      ...
      countOf(db.collection('storeOrders')),
      countOf(db.collection('storeOrders').where('status', '==', 'CONFIRMED'))
    ]);
    ...
    return sendJson(response, 200, {
      ...
      cards: {
        ...
        storeOrders: storeOrdersTotal,
        storeOrdersAwaitingShipment: storeOrdersOpen
      },
      byKind,
      revenue30d: revenue,
      note: 'Store orders are mirrored from Shopify into storeOrders. See the Store tab.'
    });
  } catch (error) {
