...
const { getDb, fieldValue, serverTimestamp } = require('../../../lib/firebase');
...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    /* The history is what the sender sees when they follow their number:
       each status with the moment it changed. Who changed it stays inside
       the panel. */
    await ref.set({
      status,
      ...
      handledNote: note,
      handledAt: at,
      history: fieldValue().arrayUnion({ status, at }),
      updatedAt: serverTimestamp()
    }, { merge: true });

    audit.record(who, { module: 'submissions', action: 'status', subject: reference, detail: `Moved to ${status}` }, request);
