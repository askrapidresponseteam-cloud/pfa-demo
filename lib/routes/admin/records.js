...
module.exports = async function handler(request, response) {
  ...
  if (!TYPES[type]) {
    return sendJson(response, 400, {
      code: 'BAD_TYPE',
      message: 'type must be members, caretakers, submissions or payments.'
    });
  }
  ...
  try {
    ...
    const newestFirst = type === 'submissions';
    const orderField = newestFirst ? 'receivedAtMs' : '__name__';
    ...
    if (query.cursor) {
      ref = newestFirst
        ? base.orderBy(orderField, 'desc').startAfter(Number(query.cursor)).limit(limit + 1)
    ...
    const shape = {
      ...
      submissions: submissionRow, payments: paymentRow
    }[type];
    ...
    return sendJson(response, 200, {
      ...
      cursor: docs.length
        ? (newestFirst ? docs[docs.length - 1].data().receivedAtMs : docs[docs.length - 1].id)
        : null
    });
  } catch (error) {
