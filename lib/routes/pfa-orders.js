...
/* The codes the map above actually issues. The two-letter fast path used to
   return whatever two letters it was given, so province "ZZ" went to Shopify
   as the delivery province of a real order. */
const PROVINCE_CODES = new Set(Object.values(INDIA_PROVINCE_CODES));

function provinceCode(value) {
  const normalized = cleanText(value, 80).toLowerCase().replace(/\s+/g, ' ');
  if (/^[a-z]{2}$/i.test(normalized)) {
    const given = normalized.toUpperCase();
    if (PROVINCE_CODES.has(given)) return given;
    throw checkoutError('Choose a valid Indian state.', 'INVALID_ADDRESS');
  }
