...
const CAREGIVER_ID = /^PFA-CCT-[A-Z0-9]{8}$/;
...
module.exports = async function handler(request, response) {
  ...
  if (!CAREGIVER_ID.test(id)) {
    return sendJson(response, 400, {
      code: 'INVALID_ID',
      message: 'That is not a PFA card number. They look like PFA-CCT-XXXXXXXX.'
    });
  }

  try {
    // Colony caregiver cards keep a public projection of their own, precisely so a
    ...
    return sendJson(response, 200, {
      found: true,
      kind: 'caregiver',
      cardType: 'Colony Animal Colony caregiver',
      cardId: id,
      name: card.name || '',
