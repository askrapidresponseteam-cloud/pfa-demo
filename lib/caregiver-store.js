...
async function queueEmail({ template, to, payload, dedupeKey }) {
  const db = getDb();
  const id = dedupeKey
    ? `${template}_${CAREGIVER.hashToken(dedupeKey).slice(0, 32)}`
    : CAREGIVER.createEventId();
...
/* Filtering on status and ordering on nextAttemptAt in one query needs a
   ...
   what is queued or retrying, which a single-field index answers, and the
   due-time check and the ordering are done here. */
async function claimQueuedEmails(limit) {
  ...
  return snapshot.docs
    ...
    .filter((item) => !item.nextAttemptAt || String(item.nextAttemptAt) <= now)
...
async function recordEmailResult({ emailId, ok, error, providerId }) {
  ...
  await ref.update({
    ...
    attempts,
    lastError: CAREGIVER.clean(error, 300),
