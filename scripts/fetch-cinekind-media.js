...
/* A dropped socket is weather, not a verdict. Three fresh connections with
   a pause between them, and only then is the file called failed. */
...
async function download() {
  ...
  for (const name of names) {
    ...
    /* Resume rather than restart: a file already on disk at a plausible size
       is kept, so a re-run after one failure chases only what is missing and
       spends none of the Federation's bandwidth on what already arrived. */
    if (fs.existsSync(file) && fs.statSync(file).size >= 1024) {
      got.push(`${name}.jpg (kept, ${Math.round(fs.statSync(file).size / 1024)} KB)`);
      continue;
    }
    try {
      const body = await getWithRetry(`${HOST}/${name}.jpg`);
      ...
      failed.push(`${name}.jpg — ${error.message} (after 3 attempts)`);
    }
  }
  ...
  (async () => {
    ...
    if (process.argv.includes('--rewrite')) {
      if (failed.length) {
        ...
        console.log('Everything that did download is kept, so re-running this command');
        console.log('retries only the missing files.');
        process.exitCode = 1;
        return;
      }
      rewrite();
