...
function checkProjects() {
  ...
  /* A project this site has left is refused before anything else is compared:
     on 7 Sep 2026 the key for pfa-oldsite was picked up off a Desktop by its
     timestamp, and "fixing" the browser to match it would have been the worse
     mistake. The list lives in scripts/firebase-project.js. */
  const { RETIRED, treeReferences } = require('./firebase-project');
  for (const [label, value] of [['Browser project', web], ['Server project', server]]) {
    if (value && RETIRED.includes(value)) {
      bad(label, `"${value}" is a project this site has left`,
        'Use the key and the web config of the current project, and not this one anywhere.');
      return;
    }
  }

  /* The deploy targets must name the browser's project too, or `firebase
     deploy` sends the rules somewhere the sign-in never touches. */
  const others = treeReferences().filter((r) => r.project && r.project !== web);
  if (others.length) {
    bad('Deploy targets', others.map((r) => `${r.where.replace(/\s+/g, ' ')} says "${r.project}"`).join('; '),
      `The browser signs in to "${web}". Make .firebaserc, package.json and assets/firebase-config.js name the same project.`);
  } else {
    ok('Deploy targets', `.firebaserc and package.json name ${web}`);
  }
  ...
  if (web === server) {
    ...
    bad('Projects do not match', `browser signs in to "${web}", server holds "${server}"`,
      ...
      + 'Use the service account key of the project the browser signs in to '
      + '(node scripts/firebase-project.js /path/to/key.json shows which project a key belongs to).');
  }
}
