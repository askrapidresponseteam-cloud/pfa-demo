...
(function () {
  ...
  var CSS = [
    ...
    ".pfa-menu{position:fixed;inset:76px 0 0 0;background:#fff;z-index:999;overflow-y:auto;overscroll-behavior:contain;opacity:0;visibility:hidden;pointer-events:none;transition:opacity .35s cubic-bezier(.2,.6,.2,1),visibility 0s linear .35s}",
    ...
    ".pm-k{font-family:var(--font-s,'Marcellus',Georgia,serif);font-size:11px;font-weight:400;letter-spacing:.24em;text-transform:uppercase;color:var(--mut-2,#7A848D);margin:0 0 10px;line-height:1.2}",
    ".pm-g a{display:block;font-family:var(--font-d,'Archivo',system-ui,sans-serif);font-weight:700;font-size:15px;letter-spacing:.02em;color:var(--ink,#0E1116);text-decoration:none;line-height:1.35;padding:11px 0;border-bottom:1px solid var(--hair-soft,rgba(14,17,22,.08));transition:color .25s cubic-bezier(.2,.6,.2,1)}",
    ".pm-g a:hover{color:var(--blue-ink,#006DB3)}",
    ".pm-g a[aria-current=page]{color:var(--blue-ink,#006DB3)}",
    ...
    ".pfa-menu a.pfa-btn:hover{color:var(--ink,#0E1116);border-color:var(--blue,#00A4FF)}",
    ".pfa-menu a.pfa-btn span{position:relative;z-index:1}",
    ".pm-meta{font-family:var(--font-s,'Marcellus',Georgia,serif);font-size:12px;letter-spacing:.08em;color:var(--mut,#55606A)}",
    ...
    ".pfa-menu a:focus-visible{outline:2px solid var(--blue-ink,#006DB3);outline-offset:3px}",
