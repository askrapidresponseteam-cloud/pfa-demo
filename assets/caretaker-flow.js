/* Colony Animal Caretaker Card - the application journey.
...
(function () {
  ...
  function onIssued(data) {
    ...
    if (!data.sample) P.store('pfa_caretaker', {
    ...
    if (data.alreadyHeld) {
      ...
      notice('<strong>Someone at this address already holds a card.</strong>'
        ...
        + 'use the first card &mdash; and if the printed card was lost, <a href="lost-card.html">order a replacement</a>.');
    }
  ...
  if (share) {
    share.addEventListener('click', function () {
      ...
      if (!url) return;
      if (navigator.share) return navigator.share({ title: 'My Caretaker Card', url: url }).catch(function () {});
      P.copy(url);
    });
  }
  ...
  window.PFACaretakerFlow = { onIssued: onIssued, cardData: cardData, paint: paint };
})();
