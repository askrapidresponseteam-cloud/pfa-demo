(function () {
  ...
  var STEPS = [
    ...
    { key: 'paid', title: 'Payment verified', copy: 'The seller confirmed the payment.' },
  ...
  function render(order) {
    ...
    var notice = order.status === 'CANCELLED' ? '<div class="notice amber">This order was cancelled by the seller.</div>' :
  ...
  function show(id) {
    if (!id) return;
    out.innerHTML = '<div class="notice">Checking with the seller…</div>';
    fetch('/api/pfa-order-status?id=' + encodeURIComponent(id), { headers: { Accept: 'application/json' } })
      ...
      .then(function (res) {
        ...
        out.innerHTML = res.status === 404 ?
          '<div class="notice amber">We could not find a verified order with that number.</div>' :
