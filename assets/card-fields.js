...
(function (root, factory) {
  ...
  var CARETAKER_CARD = 'https://peopleforanimalsindia.org/caretaker-card.html?id=';
  var CARETAKER_SPECIMEN = 'PFA-CCT-SPECIMEN';

  function caretaker(raw, now) {
    var data = raw || {};
    var today = now || new Date();
    var missing = [];
    var ghost = {};
    ...
    if (!contact.length) {
      /* A caregiver card with no way to reach the caregiver defeats the point
    ...
    return {
      ...
      /* Never empty: an unissued card carries a specimen code rather than a
         ...
         exist. */
      qr: clean(data.qr) || CARETAKER_CARD + encodeURIComponent(cardId || CARETAKER_SPECIMEN),
