...
PFA.formData=function(form){
  ...
  new FormData(form).forEach(function(v,k){
    if(typeof v!=='string')return; /* files travel separately, shrunk first */
    if(R)v=R.normaliseField(k,v);
...
/* A photograph from a phone is 3-8 MB; the report needs the picture, not
   the megabytes. Shrunk here to a JPEG no wider than maxEdge before it is
   sent, so three photos travel in under a megabyte and nothing is ever
   uploaded at full size. */
PFA.shrinkImage=function(file,maxEdge,quality){return new Promise(function(resolve,reject){
if(!file||!/^image\//.test(file.type))return reject(new Error('Choose a photo (JPEG, PNG or WebP).'));
var reader=new FileReader();reader.onerror=function(){reject(new Error('That photo could not be read.'))};
reader.onload=function(){var img=new Image();img.onerror=function(){reject(new Error('That photo could not be read.'))};
img.onload=function(){var limit=maxEdge||1600,ratio=Math.min(1,limit/Math.max(img.width,img.height)),c=document.createElement('canvas');
c.width=Math.max(1,Math.round(img.width*ratio));c.height=Math.max(1,Math.round(img.height*ratio));var x=c.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,c.width,c.height);x.drawImage(img,0,0,c.width,c.height);
resolve(c.toDataURL('image/jpeg',quality||0.82))};img.src=String(reader.result||'')};reader.readAsDataURL(file)})};
/* The photo control: up to `max` pictures, each shrunk on arrival, with a
   thumbnail and a remove button. photos() returns what is attached. */
PFA.photoControl=function(host,max){var list=[],cap=max||3,input=PFA.q('input[type=file]',host),strip=PFA.q('[data-photo-strip]',host),note=PFA.q('[data-photo-note]',host);
function paint(){if(strip)strip.innerHTML=list.map(function(p,i){return '<span class="photo-thumb"><img alt="Photo '+(i+1)+'" src="'+p+'"><button type="button" aria-label="Remove photo '+(i+1)+'" data-photo-remove="'+i+'">\u00d7</button></span>'}).join('');
if(note)note.textContent=list.length?list.length+' of '+cap+' photos attached.':'';if(input)input.disabled=list.length>=cap}
if(input)input.addEventListener('change',function(){var files=[].slice.call(input.files||[]).slice(0,cap-list.length);input.value='';if(!files.length)return;if(note)note.textContent='Preparing\u2026';
files.reduce(function(chain,f){return chain.then(function(){return PFA.shrinkImage(f,1600,0.82).then(function(d){list.push(d)},function(e){PFA.toast(e.message)})})},Promise.resolve()).then(paint)});
if(strip)strip.addEventListener('click',function(e){var b=e.target.closest('[data-photo-remove]');if(!b)return;list.splice(Number(b.dataset.photoRemove),1);paint()});
return {photos:function(){return list.slice()},clear:function(){list=[];paint()}}};
PFA.saveSubmission=function(kind,data,extras){var payload=JSON.stringify({kind:kind,data:data,page:location.pathname,photos:(extras&&extras.photos)||[]});
...
return send()};
PFA.followUrl=function(ref){return 'track.html#ref='+encodeURIComponent(ref)};
...
{type:'Page',title:'Home',body:'People for Animals: local contacts, adoption, casework and learning',href:'index.html'},
{type:'Help',title:'Help and contacts',body:'PFA local contacts by state, nearest first, what not to do while you find a vet, report with a photo',href:'help.html'},
{type:'Page',title:'Contacts and Help Desk',body:'Find a PFA local contact, raise a complaint or ask a question',href:'network.html'},
{type:'Page',title:'Stories',body:'Rescue, recovery, adoption and caregiver stories',href:'stories.html'},
{type:'Page',title:'Newsroom',body:'Verified animal welfare cases and the record of how they moved',href:'newsroom.html'},
...
{type:'Page',title:'Get Involved',body:'Volunteer roles, community caregiver applications and learning',href:'get-involved.html'},
{type:'Page',title:'Give',body:'Donate to PFA rescue, legal work and learning',href:'give.html'}
];
...
(D.units||[]).forEach(function(x,i){out.push({type:'PFA contact',title:x.c+', '+x.s,body:x.h+' '+(x.a||''),href:'hospital.html?id='+i})});
return out}
...
(function(){
  ...
  function closeOverlay(){
    ...
    if(modal){var modalClose=modal.querySelector('[data-modal-close]');if(modalClose)modalClose.click();else modal.classList.remove('open');return true}
    var search=document.querySelector('#searchOverlay.open,[data-search-overlay].open');
    if(search){var searchClose=search.querySelector('[data-search-close]');if(searchClose)searchClose.click();return true}
    var menu=document.querySelector('#mobileMenu.open');
    if(menu){var menuClose=menu.querySelector('[data-menu-close]');if(menuClose)menuClose.click();else menu.classList.remove('open');return true}
    return false
  }
