/* People for Animals - Colony Animal Caretaker Card renderer.
...
(function () {
  ...
  /* Matte white, the same treatment as the Patron card.
     ...
     grey and a caretaker holding it up to a police officer at night gets
  ...
  /* Shrink first, wrap second: names and addresses must not clip on an ID
     card, so the type gets smaller until the text fits the allowed lines. */
  function fitWrapped(ctx, text, px, maxMm, maxLines, scale, stack, weight) {
    var size = px;
    setFont(ctx, size, scale, stack, weight);
    var lines = wrap(ctx, text, maxMm * scale);
    while (lines.length > maxLines && size > px * 0.6) {
      size -= px * 0.05;
      setFont(ctx, size, scale, stack, weight);
      lines = wrap(ctx, text, maxMm * scale);
    }
  ...
  function buildPdf(pages) {
    ...
    /* Pages default to the caretaker card size; a page may carry its own
  ...
  function complete(data) {
    ...
    var filled = window.PFACardFields.caretaker(data);
    var out = {};
  ...
  function fileStem(data) {
    var id = String(data.cardId || 'caretaker-card').replace(/[^A-Za-z0-9-]/g, '');
    return id || 'caretaker-card';
  }
