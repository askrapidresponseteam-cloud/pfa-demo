# Colony Animal Caretaker Card - journey and data architecture
...
1. **Open** (`caretaker.html`) - the card, one line of copy, **Begin**. The
...
3. **Issued** - `POST /api/caretaker/apply` issues a free card immediately and
...
6. **Pay** - `POST /api/caretaker/order` → CCAvenue → `/api/payment/response`.
...
| `caretakerMobileIndex/{mobile}` | uniqueness index, one active card per person | issuance |
| `caretakerEmails/{id}` | outbound queue: template, payload, attempts, lastError | every notifiable event |
...
not the holder. The public page therefore reads exactly one document - `caretakerPublic` - with no joins or queries. `/api/caretaker/card` sets
...
## Shipment state machine (`lib/caretaker.js`)
...
`/api/caretaker/email-worker` (Vercel cron, every 10 min) with exponential
...
- **Price**: `SHIPPING_PRICE` is a server constant. The browser never carries an
  amount; `/api/caretaker/order` builds the CCAvenue request itself.
...
| Method | Path | Auth |
|---|---|---|
| POST | `/api/caretaker/apply` | none (public, free issuance) |
| GET | `/api/caretaker/card?id=` | none, cached |
| POST | `/api/caretaker/order` | card token |
| POST | `/api/caretaker/admin-shipment` | `PFA_ADMIN_TOKEN` |
| GET/POST | `/api/caretaker/email-worker` | `PFA_ADMIN_TOKEN` / `CRON_SECRET` |
...
`lost-card.html` → `POST /api/caretaker/replace`.
...
| Layer | Key | Behaviour |
|---|---|---|
| Hard | `caretakerMobileIndex/{mobile}` | Blocks. Returns the held card, no token. |
| Soft | `caretakerIdentityIndex/{identityKey}` | Warns. Records `softDuplicateOf`. |
...
- Rate limiting on `/api/caretaker/apply` is not implemented. Free issuance is
